...
## Now that we have habitat information for all mammals and birds in Central and South American countries of interest, we need to figure out which are frugivores. We do this by using the EltonTraits Database (Wilman et al. 2014)
...
# Some of the resulting rows had no corresponding species in the EltonTraits dataset (NAs in these rows), but can be filled in manually (e.g., after correcting for changes in taxonomy and spelling).

# Will eventually remove species that have lower than a 10% frugivorous diet after resolving the issues with naming conventions. This code is necessary to figure out alternate names for species with NA values for Elton Traits. 

## Resolving Taxonomies
# Mammals

# Find species that did not match between the IUCN and EltonTraits
...
mamm_trait_alternates <- merge(mamm, mamm_lookup_subset, by.x=c("scientific_name"), by.y=c("elton_species_name"))
...
# Bind the alternate name object to the original edited merge. The scientific name column is the elton_species_names.
...
# Merge list of alternate names with elton_traits to append them to the original merge 
bird_trait_alternates <- merge(birds, bird_lookup_subset, by.x=c("scientific_name"), by.y=c("elton_species_name"))
