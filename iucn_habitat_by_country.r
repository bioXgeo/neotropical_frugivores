...
south_america <- GET(url = paste0(api_url, 'region/list?token=', token))
south_america <- fromJSON(content(south_america, as = 'text'), simplifyDataFrame = TRUE, flatten = TRUE)$result
rl_countries(key=token)
south_america <- GET(url = paste0(api_url, 'species/region/:south_america/page/:page_number?token=', token))
south_america <- fromJSON(content(south_america, as = 'text'), simplifyDataFrame = TRUE, flatten = TRUE)$result
...
all_habitat_2 <- co_ec_spp %>%
  rowwise %>%
  do(get_habitat(.$taxonid))

setwd("/Users/bethgerstner/Desktop/MSU/Zarnetske_Lab/Data/IUCN_Data")
write.csv(all_habitat_2, 'habitat_by_species_fixed.csv', row.names = FALSE)
